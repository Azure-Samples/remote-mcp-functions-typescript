 ...
 *
 * Demonstrates duck-typing and CallToolResult examples:
 * - direct ImageContentBlock
 * - ContentBlock[]
 * - CallToolResult with structuredContent
 */

import { app, InvocationContext, arg } from '@azure/functions';
import type { CallToolResult, ImageContentBlock, TextContentBlock } from '@azure/functions';
...
export async function getImageWithText(
    ...
    const content: [TextContentBlock, ImageContentBlock] = [
        new TextContentBlockValue('Here is the Azure Functions logo'),
        new ImageContentBlockValue(base64ImageData, 'image/png'),
    ];
